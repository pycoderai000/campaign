...
export default function ContentCalendar({ deliverables, campaignName }: ContentCalendarProps) {
  ...
  const getDeliverablesForDate = (date: number) => {
    ...
    return deliverables.filter(d => d.postingDate === dateStr);
  };
        ...
        <div className="min-w-full">
          ...
          <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2">
            {dayNames.map((day) => (
              <div
                key={day}
                className="text-center text-xs sm:text-sm font-bold text-gray-600 py-2"
              >
                {day}
              </div>
            ))}
